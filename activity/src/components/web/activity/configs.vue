		...
		<div class="config-contents">
			...
			<div class="content" id="condition-config-content" v-show="currTabHas==configsTabs[1].url">
				<div class="label-inline">
					...
					<checkbox :model.sync="configData.player"  :disabled="true" value="M" style="margin-left: 10px;">手机号</checkbox>
					<question>使用手机号码作为参与对象，需接入短信验证码发送功能</question>
				</div>
				<div class="label-inline">
					<span class="onse">关注公众号：</span>
					<checkbox :model.sync="configData.isfollow" value="see_rank" :disabled="true">查看排名</checkbox>
					<checkbox :model.sync="configData.isfollow" value="play_game" :disabled="true" style="margin-left: 10px;">参与活动</checkbox>
					<question>使用该功能需接入指定的微信公众号进行联调</question>
				</div>
				<div class="label-inline">
					<span class="onse">领奖验证码：</span>
					<radio name="verificationcodestyle" :model.sync="configData.verificationcodestyle" value="picture">图形验证码</radio>
				...
				<div class="label-inline" v-show="showQuest">
			...
			<!-- 奖品配置 -->
			<div class="content" id="prize-config-content" v-show="currTabHas==configsTabs[2].url">
				<component :is="currentView"></component>
			</div>
			<!-- 高级配置 -->
			<div class="content" v-show="currTabHas==configsTabs[3].url">
				<advanced-config-content></advanced-config-content>
			</div>
		</div>
				...
				if(params.aid) {
					api.selectActivityById(this, {
						...
						if(back.resCode == "0") {
							...
							utils.extend(window.store.activityConfig, eval("(" + back.repBody[0].extbody + ")"), true);
