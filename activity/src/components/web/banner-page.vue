<template>
	<div class="banner-page-view">
		<div class="upload-view">
			<div class="left" :style="{'background-image':(bannerPage.imgUrl!=''?'url('+bannerPage.imgUrl+')':1)}">
...
<script>
	export default {
		data(){
			var bannerPage = window.store.activityConfig.bannerPage;
			return{
				bannerPage,
				defaultImg : window.store.activityConfig.bannerPage.imgUrl.toString()
			}
		},
		components: {
		...
		methods: {
			...
			setDefaultImg(){
				this.bannerPage.imgUrl=this.defaultImg;
			}
		},
		events: {
			onFileChange(file) {
