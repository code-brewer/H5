			...
			<table class="table table-bordered table-striped">
				<thead>
					<tr>
						...
						<th>奖品名称</th>
						<th>奖品类型</th>
						<th v-show="!setPrize">入库时间</th>
						<th v-show="!setPrize">兑奖方式</th>
						<th>兑奖时间</th>
				...
				<tbody>
					<tr v-for="item in List | filterBy pname in 'pname' |Between 'ctime' beginDate endDate | limitBy 50 (pagerConfig.pageNum*50-50) ">
						...
						<td>{{item.pname}}</td>
						<td>{{item.types}}</td>
						<td v-show="!setPrize">{{item.ctime.split(' ')[0]}}</td>
						<td v-show="!setPrize">{{item.cashprize=='offline'?'线下兑奖':'线上兑奖'}}</td>
						<td v-show="!setPrize">{{ item.startenabled.split(' ')[0] + ' 至 ' + item.endenabled.split(' ')[0] }}</td>
						<td v-show="setPrize">{{ item.startenabled.split(' ')[0] + ' 至 ' + item.endenabled.split(' ')[0] }}</td>
						<td v-show="!setPrize">
							<a class="btn btn-default" @click="del(item.gid)" role="button">删除</a>
						</td>
					</tr>
				</tbody>
			</table>
		...
		<modal :show.sync="show" size="md">
			<div class="modal-user-body">
				...
				<form class="modal-user-box">
					<p>
						<label>奖品名称：</label> <input type="text" v-model="prize.pname" class="form-control" placeholder="请输入奖品名称" name="account">
					</p>
					<p>
						<label>奖品类型：</label>
					...
					<p>
						<label>奖品数量：</label>
						<input type="text" class="form-control" v-model="prize.count" placeholder="数量" style="width: 150px;" name="account">
					</p>
					<p>
						<label>兑奖方式：</label>
						<radio name="ashprize" :model.sync="cashprize" :checked="true" value="true" style="width: 80px;text-align: left;">线下兑奖</radio>
						<radio name="ashprize" :model.sync="cashprize" value="false" style="width: 80px;text-align: left;margin-left: 10px;">线上兑奖</radio>
					</p>
					<p>
						<label>兑奖提示：</label>
						<input type="text" class="form-control" v-model="prize.des" placeholder="凭兑奖券号到XXX扫码领取" name="account">
					</p>
					<p>
						<label>券码生成：</label>
						<radio name="isAuto" :model.sync="isAuto" :checked="true" value="false" style="width: 80px;text-align: left;">系统生成</radio>
						<radio name="isAuto" :model.sync="isAuto" value="true" style="width: 80px;text-align: left;margin-left: 10px;">手动输入</radio>
					</p>
					<p v-show="isAuto=='true'">
						<label></label>
						<input maxlength="4" type="text" class="form-control" v-model="prize.sidPre" placeholder="请输入1-4位券码" name="券码">
					</p>
					<p>
		...
		data() {
			...
			return {
				...
				prize: {
					...
					count: 1,
					sidPre: '',
					startenabled: "2016-08-01", //奖品有效开始时间  注意格式 后面是00点00分00秒
					endenabled: "2016-08-09", //奖品有效结束时间 注意格式 后面是23点59分59秒
