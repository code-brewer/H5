...
$.Jcrop = function(obj,opt)
{
	...
	var Selection = function()/*{{{*/
	{
		...
		{
			for(var i in li) handle[li[i]] = insertHandle(li[i]);
		};
		/*}}}*/
	...
	$.fn.zyUpload = function(options,param){
		var ZYFILE = {
					...
					//			formdata.append("tailor", $("#uploadTailor_"+file.index).attr("tailor"));	
								if($("#uploadTailor_"+file.index).attr("tailor")!=false)
								{
									var att= $("#uploadTailor_"+file.index).attr("tailor");
									if(att!="false"){
										var json= eval("("+$("#uploadTailor_"+file.index).attr("tailor")+")");
										formdata.append("x", json.leftX);
										formdata.append("y",json.leftY);
										formdata.append("heigh",json.height);
										formdata.append("width",json.width);
									}else{
										formdata.append("x", "");
										formdata.append("y","");
										formdata.append("heigh","");
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					html += '<form id="uploadForm" action="'+para.url+'" method="post" enctype="multipart/form-data">';
					html += '	<div class="upload_box">';
					html += '		<div class="upload_main single_main" style="    border-width:0px!important">';
		            html += '			<div class="status_bar" style="border-top: 0px">';
		            html += '				<div class="btns" style="left:0">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		            html += '					<div class="webuploader_pick">选择文件</div>';
		            html += '					<div class="upload_btn">开始上传</div>';
		            html += '				</div>';
		            html += '			</div>';
		            html += '			<div id="preview" class="upload_preview" style="border-bottom:0px;text-align: left;">';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var type = newStr.split(".")[0].split("").reverse().join("");
				// 处理不同类型文件代表的图标
				var fileImgSrc = "./dist/static/img/fileType/";
				if(type == "rar"){
...
//						});
			...
			this.createCorePlug = function(){
		 		var that= this;
				var params = {
					fileInput: $(that).find("#fileImage").get(0),
					uploadInput: $(that).find("#fileSubmit").get(0),
					dragDrop: $(that).find("#fileDragArea").get(0),
					url:$(that).find("#uploadForm").attr("action"),
						...
						var funDealtPreviewHtml = function() {
							...
							if (file) {
								...
								reader.onload = function(e) {
									// 处理下配置参数和格式的html
									if(para.onlyFile) t.onDelete(t.uploadFile[0], []);
									t.uploadFile=[file];
								...
								funAppendPreviewHtml(html,that);
							}
						};
						
						// 添加预览html
						var funAppendPreviewHtml = function(html,that){
							// 添加到添加按钮前
							if(para.dragDrop){
								$(that).find("#preview").append(html);
							}else{
								$(that).find(".add_upload").before(html);
							}
							// 绑定删除按钮
							funBindDelEvent();
							funBindHoverEvent();
...
//								}
					...
					onDelete: function(file, files) {
						...
						// 移除效果
						$(that).find("#uploadList_" + file.index).fadeOut();
						// 重新设置统计栏信息
					...
					onProgress: function(file, loaded, total) {
						var eleProgress = $(that).find("#uploadProgress_" + file.index), percent = (loaded / total * 100).toFixed(2) + '%';
					...
					onSuccess: function(file, response) {
					    var flag=para.onSuccess(file, response);  // 回调方法
						if(flag)
						{
							this.funDeleteFile(file.index, false);
							$(that).find("#uploadProgress_" + file.index).hide();
							$(that).find("#uploadSuccess_" + file.index).show();
						}else{
							this.onFailure(file, false);
						}
						...
						if($(that).find("#uploadTailor_"+file.index).length>0){
							$(that).find("#uploadTailor_" + file.index).hide();
						}
					},
					onFailure: function(file, response) {
						para.onFailure(file, response);  // 回调方法
						$(that).find("#uploadProgress_" + file.index).hide();
						$(that).find("#uploadFailure_" + file.index).show();
						if($(that).find("#uploadTailor_"+file.index).length>0){
							$(that).find("#uploadTailor_" + file.index).hide();
						}
//						$("#uploadInf").append("<p>文件" + file.name + "上传失败！</p>");	
			...
			this.addEvent = function(){
				var that =this;
				// 如果快捷添加文件按钮存在
				if($(this).find(".filePicker").length > 0){
					// 绑定选择事件
					$(this).find(".filePicker").bind("click", function(e){
		            	$(that).find("#fileImage").click();
		            });
				}
	            
				// 绑定继续添加点击事件
				$(this).find(".webuploader_pick").bind("click", function(e){
	            	$(that).find("#fileImage").click();
	            });
				 
				$(this).find(".upload_btn").bind("click", function(e){
					// 判断当前是否有文件需要上传
					if(ZYFILE.funReturnNeedFiles().length > 0){
						$(that).find("#fileSubmit").click();
					}else{
						alert("请先选中文件再点击上传");
					}
	            });
				...
				// 如果快捷添加文件按钮存在
				if($(this).find("#rapidAddImg").length > 0){
					// 绑定添加点击事件
					$(this).find("#rapidAddImg").bind("click", function(e){
						$(that).find("#fileImage").click();
		            });
				}
			};
