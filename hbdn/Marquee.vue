<template>
	<div class="app-view game-page">
		<section class="section-wrap">
            ...
            <div class="prize-wrap" id="lottery" style="margin: 35% auto;">
    ...
    <!-- 遮罩层  -->
    <section class="bg-section"  v-if="alertTip.rootTip" style="background-color: rgba(0,0,0,0.5);">
...
<section class="model" style="position: inherit;margin-top: 150px;" v-if="alertTip.Atip">
            <div class="panel">
                <div class="msg">
                    <div class="messagebg">
                        <h2 style="padding-bottom: 5%">抽奖结果</h2>
                        <p style="text-align: center;padding: 0 50px;line-height: 65px;">恭喜抽中{{lotteryObj.prizename}}我们会在2个工作日内为您充值,请注意查收!</p>
                        <a class="btn" v-link="{path: '/home'}" style="background: url(images/btnbg.png);font-size: 32px;display: inline-block;margin-left: 60px;">
                        继续挑战
                        </a>
                          <a class="btn" v-link="{path: '/home'}" style="background: url(images/btnbg.png);font-size: 32px;display: inline-block;">
                        好的
                        </a>
...
<section class="model" style="position: inherit;margin-top: 150px;" v-if="alertTip.Btip">
            <div class="panel">
                <div class="msg">
                    <div class="messagebg">
                        <h2 style="padding-bottom: 5%">抽奖结果</h2>
                        <p style="text-align: center;padding: 0 50px;line-height: 65px;">很遗憾,差点就抽中啦!<br>再挑战一次吧!</p>
                        <a class="btn" v-link="{path: '/home'}" style="background: url(images/btnbg.png);font-size: 32px;">
                        继续挑战
                    </a>
                    <!-- <img :src="'images/xx.png'" @click="answerData.show = false" class="close"> -->
                    </div>
...
<address :show-address.sync="alertTip.showAddress"  :backdrop = "backdrop"></address>
        ...
        data() {
         return {
            ...
            ismark:false,
                ...
                alertTip:{
                    rootTip:false,
                    // showAddress:sessionStorage.getItem("login")? false : true, //有登录就不弹,没登录就弹出
                    showAddress:false,
                    Atip:false, //抽中的弹框
                    Btip:false, //没抽中的弹框
                    Ctip:false, //领取成功的弹框
                ...
                list:[],
                lottery:[
                {"img":"images/choujiangbg.png","itemimg":"images/wuyuan.png","index":0,"seq":"5yuan","title":"5yuan","haveprize":false},
                {"img":"images/choujiangbg.png","itemimg":"images/liuliang30.png","index":1,"seq":"二","title":"30M","haveprize":true},
                {"img":"images/choujiangbg.png","itemimg":"images/chadian.png","index":2,"seq":"还差一点就抽中啦","title":"还差一点就抽中啦","haveprize":true},
                {"img":"images/choujiangbg.png","itemimg":"images/liuliang50.png","index":3,"seq":"四","title":"50M","haveprize":true},
                {"img":"images/choujiangbg.png","itemimg":"images/shiyuan.png","index":4,"seq":"五","title":"10yuan","haveprize":true},
                {"img":"images/choujiangbg.png","itemimg":"images/chadian.png","index":5,"seq":"还差一点就抽中啦","title":"还差一点就抽中啦","haveprize":false},
                {"img":"images/choujiangbg.png","itemimg":"images/liuliang30.png","index":6,"seq":"七","title":"30M","haveprize":false},
                {"img":"images/choujiangbg.png","itemimg":"images/wuyuan.png","index":7,"seq":"八","title":"5yuan","haveprize":false},
               ],
                lotterys:[],
                 times1:'',//剩下的次数
                 doLuckyResult:[],
         ...
         //        this.refreshRank = !this.refreshRank;
            ...
            getActUserTimes(){
                ...
                sapi.queEveryDay(this,{},(back)=>{
                    if(back.resCode==0 || back.resCode=="0"){
                        ...
                        //     that.iclick=true;
                        // } 
                        back.repBody[0].player=='0' ? that.iclick=true : that.iclick=false;
                    }
                    else{
            ...
            userDoLucky(){
                ...
                sapi.doLucky(this,{
                    ...
                    if (!back) return;
                    // console.log(back)
                    // console.log(back.resCode)
                    if (back.resCode==='0'){
                        that.isPrize=false;
                        that.lotteryObj.prize = 2;
                        if (back.repBody[0].pid==='0'){
                            var prizeIndex;
                            that.lotteryObj.prizename = prize.pname;
                        }
                        else if (back.repBody[0].pid==='1') {
                                prizeIndex = ['0','7'];
                            that.isPrize=true;
                            console.log('奖品位置'+prizeIndex[parseInt(prizeIndex.length*Math.random())])
                            that.lotteryObj.prize =prizeIndex[parseInt(prizeIndex.length*Math.random())];
                        }else if (back.repBody[0].pid==='2'){
                             prizeIndex = ['1','6'];
                            that.isPrize=true;
                            console.log('奖品位置'+prizeIndex[parseInt(prizeIndex.length*Math.random())])
                            that.lotteryObj.prize =prizeIndex[parseInt(prizeIndex.length*Math.random())];
                        }else if (back.repBody[0].pid==='3') {
                            prizeIndex = ['4'];
                            that.isPrize=true;
                            console.log('奖品位置'+prizeIndex[parseInt(prizeIndex.length*Math.random())])
                            that.lotteryObj.prize =prizeIndex[parseInt(prizeIndex.length*Math.random())];
                        }else if (back.repBody[0].pid==='4') {
                            prizeIndex = ['3'];
                            that.isPrize=true;
                            console.log('奖品位置'+prizeIndex[parseInt(prizeIndex.length*Math.random())])
                            that.lotteryObj.prize =prizeIndex[parseInt(prizeIndex.length*Math.random())];
            ...
            isClick(){
                ...
                if(!this.iclick && this.times1>0){
                    ...
                    this.$dispatch('loading', {text: '请再来挑战', show: true, duration: 1000});
                }
                else{
