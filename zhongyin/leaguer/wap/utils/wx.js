export default {
	//注册
	config(data, showmenu) {
		console.log(data)
		// console.log('aaaaappppid-'+appid);
		// alert(appid+"-----"+data.timestamp+"----"+data.noncestr+"----"+data.signature);
		wx.config({
			debug: false,
			appId: data.appId,
		...
		var that = this;
		var weixinShare = window.memberinfo.wxshare;
		// 默认隐藏右上角
		wx.ready(function() {
			if(!showmenu) {
				wx.showOptionMenu();
				that.share(weixinShare.title,weixinShare.description,window.location.href,weixinShare.imgUrl,()=>{
					console.log('分享成功')
				})
			} else {
				wx.hideOptionMenu();
