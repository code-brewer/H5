...
var FileUploadComponent = Vue.extend({
  ...
  <button style="width:78px;height:30px;position:relative;">选择文件
  <input style="position:absolute;top:0;left:0;width:100%;height:100%;opacity: 0;" type="file" name="{{ name }}" id="{{ id || name }}" accept="{{ accept }}" v-on:click="fileInputClick" v-on:change="fileInputChange" multiple="{{ multiple }}"><slot></slot></button></div>',
  props: {
    class: String,
    name: {
      type: String,
