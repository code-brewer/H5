<template>
	<div class="box1 container-fluid" style="min-width:1005px" >
		<div class="top" style="min-width: 800px">
        	<search-time style="width:100%;" :searchdate="selectData.searchTime">
		...
		<div class="middle">
	        ...
	        <tbody>
	          <tr v-for ="item in searchlist.list">
	            <th scope="row">{{item.pkBid}}</th>
	            <td>{{item.operand}}</td>
	           <td>{{item.successNumber}}</td>
                <td>{{item.failureNumber}}</td>
                <td>{{item.operationType}}</td>
	           <td>{{new Date(item.ctime).format('yyyy-MM-dd HH:mm:ss')}}</td>
                <td>{{item.idxAcct}}</td>
	            <td><a style="cursor: pointer;" v-link="{path:'/home/manage/detail1/'+item.pkBid+'/'+item.operationType}">查看明细</a></td>
    ...
    export default {
        ...
        methods : {
            log( ) {
                ...
                api.log(this,{
                        ctime1:this.selectData.searchTime.begin,
                        ctime2:this.selectData.searchTime.end,
