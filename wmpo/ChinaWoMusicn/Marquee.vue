<template>
	<div class="app-view game-page">
		<section class="section-wrap">
			...
			<div class="logo">
				<img :src="'n/images/logo.png'" style="width: 150px;">
			</div>
			<div class="prize-wrap-new" id="lottery">
				<table class="prize-box" border="0" cellspacing="4" cellpadding="0" style="background-image: url('./n/images/prize-box-bg.png');">
					<tr>
			...
			<div class="downdiv">
				<p class="tip4" v-if="$root.cookieVip != '0000000000'">
				仅限沃音乐会员参与抽奖，非会员用户现在开通首月可免会员资费 <span @click="openRightNow">立即开通</span></p>
				<p class="tip4" v-if="$root.cookieVip == '0000000000'">
				仅限沃音乐会员参与抽奖，非会员用户现在开通首月可免会员资费 <em style="color: #CCC; border-bottom: 2px solid #ccc;">立即开通</em></p>
				<!-- <div class="">
			...
			<img :src="$root.activityBasePath+'/images/modal-r.png'" class="right"> -->
				<div class="div1" v-if="alertTip.Atip">
					<div>
						<div class="content-title">
							...
							<p v-if="lotteryObj.sid" style="line-height: 35px; font-size: 28px; color: #b42828;">（如未开通炫铃功能，将默认为您开通, 炫铃资费5元/月.）</p>
							<div style="padding: 30px;">
								<img :src="$root.activityBasePath+lotteryObj.url" style="border: 10px solid #b42828;" />
							</div>
						</div>
						...
						<a class="btn" @click="addr(lotteryObj.sid)" style="margin-top: 0px;background: url('./k/images/btn-bg.png') center center no-repeat; padding-left: 20px;">马上领奖</a>
					</div>
				...
				<div class="div2" v-if="alertTip.Btip">
					<div class="content-title2">
						...
						<div style="padding-top: 30px; padding-bottom: 0; margin-bottom: 0; height: 320px;">
							<img :src="$root.activityBasePath+'/images/thank.jpg'" style="border: 10px solid #b42828;" />
						</div>
					</div>
					<a class="btn" @click="buyTicket" style="background: url('./n/images/btn-bg.png') center center no-repeat; margin-top: 50px;padding-left: 25px;">一键购票</a>
				...
				<div class="div4" v-if="alertTip.Dtip">
					...
					<div style="padding: 50px 0;">
						...
						<img style="vertical-align: middle;" :src="'./n/images/lele.png'" alt="">
					</div>
				...
				<div class="div1" v-if="alertTip.Ftip">
					<div class="content-title">
						...
						<img :src="'n/images/agree.png'"/>
					</div>
					<a class="btn" @click="sureBuyLing" style="margin-top: 0px;background: url('./n/images/btn-bg.png') center center no-repeat; margin-left: 15px;">确定</a>
					<p class="border"></p>
					<div class="tip-ling">
				...
				<div class="div1" v-if="alertTip.Gtip">
					<div class="content-title">
						...
						<div class="tip-success">
							<p>请登录沃音乐<span>“个人中心”</span> 设置炫铃吧！</p>
							
						</div>
						</div>
					<a class="btn" @click="hideAlertTip()" style="margin-top: 50px;background: url('./n/images/btn-bg.png') center center no-repeat; margin-left: 15px;">确定</a>
				</div>
				<div class="btn-close" @click="hideAlertTip()"><img :src="'n/images/close.png'" /></div>
			</div>
		</section>
	...
	export default {
		...
		methods: {
			...
			saveUserH5Info(){
				...
				sapi.saveUserH5Info(that, {types: '拼歌单达人',scoreorg:1}, (back)=>{
			...
			sureBuyLing(){
				...
				sapi.findUserBasInfo(that,{'accessToken':that.$root.accesstoken},(back)=>{
			...
			qryUserBasInfo(){
				...
				that.interfacepos++;
				sapi.qryUserBasInfo(that,{'accessToken':that.$root.accesstoken},(back)=>{
			...
			buyTone(){
				...
				var body={
					...
					toneID: that.lotteryObj.sid ,
					mobile: that.lphone,
					aid: aid,
					toneType:2,
				};
				that.interfacepos++;
				sapi.buyTone(that,body,(back) =>{
					that.$dispatch('cancelloading', 'buyToneMarquee');
					if(back.resCode=='0'){
						that.alertTip.rootTip = true;
						that.alertTip.Ftip = false;
						that.alertTip.Gtip = true;
					} else {
						if(that.interfacepos<that.interfacecount){
							setTimeout(that.buyTone,1000);
						}else{
							that.alertTip.rootTip = false;
							that.alertTip.Ftip = false;
