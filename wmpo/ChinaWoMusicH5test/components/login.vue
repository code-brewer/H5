		...
		<div class="components-login">
			<div class="box" style="background: url('./H5/images/modal-bg.png') center center no-repeat;  background-size: 100% 100%;">
				...
				<div class="login-panel">
					<div class="phone">
						<span>手机号码</span>
						<input type="text" v-model="login.mobile"  placeholder="请输入联通手机号码" maxlength="11" style="width: 310px;"/>
					</div>
					<div class="code">
						<span>&nbsp;验证码</span>
						<input type="text" v-model="login.code" maxlength="6"  placeholder="短信验证码" style="width: 160px;"/>
						<a  @click="getCode()" :class="{graycode:codeing}" >
							{{timeflag}}
						</a>
					</div>
				</div>
				<a class="confirm-btn" style="background: url('./H5/images/login-btn.png') center center no-repeat;" @click="loginFun()"></a>
				<img class="close-img" :src="'H5/images/close.png'" @click="loginShow = false">
			</div>
		</div>
	</modal>
</template>
			...
			.login-panel{
				...
				.code{
					...
					a{
						...
						// border-radius: 6px;
						vertical-align: top;
						background-color: rgba(218,220,219,.5);
