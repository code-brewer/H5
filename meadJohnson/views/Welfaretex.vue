	...
	<div class="myCointxt main">
		<section class="texbody">
			...
			<div style="color: #3ea4b0; text-align: left;">
				<p style="font-size:27px;line-height:65px;">获奖两周内花妈会安排礼品寄送,注意查收哦!</p>
				<p style="font-size:22px;">规则如果疑问,请在公众号对话框以 "客服+你的问题" 的格式将你的问题告诉花妈,花妈会争取尽快回复哒!</p>
			</div>
		</section>
		...
		methods:{
			...
			submit(e) {
				...
				var bool = true,
					options = {};
				for(let i in this.options){
					if (this.options[i].value === '') {//虚拟产品手机号码必填，其他选填
				...
				this.$dispatch('loading');
				api.scoreExchange(this, options, (data) => {
					this.$dispatch('cancelLoading');
					this.isClick = false;
					if (data.resCode == "0") {
						if (this.type.match(/invented/) || this.type == "entity") {//兑换
							this.$router.replace({path: '/Conchwelfare/'+this.type});
						} else {//抽奖领奖
