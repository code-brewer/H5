<template>
    <div class="group_address_details">
        <div>
            <div class="address">
                <ul>
                    <li>
                    ...
                    <li>
                        <p>地址</p>
                        <span>
                        <select v-model = "list.province" @change = "getcity()" >
                            <option selected="selected" >{{list.province}}</option>
                            <option v-for="province in provinceArr" >{{province}}</option>
                        </select>
                        <select v-model = "list.city" @change = "getarea()" >
                            <option selected="selected" >{{list.city}}</option>
                            <option v-for="city in cityArr" >{{city}}</option>
                        </select>
                        <select v-model = "list.area" >
                            <option selected="selected" >{{list.area}}</option>
                            <option v-for="area in areaArr" >{{area}}</option>
                        </select>
                    </span>
                    </li>
                    <li>
                    ...
                    <li style="border-bottom: 0px solid #7B7B7B;">
                        <p>地址类别</p>
                        <span>
                        <select  v-model = "list.tab">
                            <option selected="selected" >{{list.tab}}</option>
            ...
            <div>
                <a class="btn" @click="addAddress()">确认</a>
            </div>
        </div>
        <!-- 温馨提示弹窗 -->
...
export default {
    ...
    data() {
        return {
            ...
            list: {
                ...
                area: '请选择', //（区域）
                address: '', //（详细地址）
                status: '0', // （是否为默认地址   0-否  1-是）
                tab: '请选择', // （地址标签 1-公司，2-家庭）
            },
            provinceArr: [],
            cityArr: [],
            areaArr: [],
    ...
    created() {
        if (this.$route.params.id) {
            ...
            let data = window.localStorage.getItem('addresslist');
            if (data) {
        ...
        */
        addAddress() {
            if (/[^\u4e00-\u9fa5]/.test(this.list.name)) {
                ...
                this.text = '请输入的姓名';
                return;
            } else if (!this.list.mobile) {
                this.show = true;
                this.text = '请输入手机号';
                return;
            } else if (this.list.province === '请选择' || this.list.city === '请选择' || this.list.area === '请选择') {
                this.show = true;
                this.text = '请选择地址';
                return;
                ...
                this.text = '请输入详细地址';
                return;
            } else if (this.list.tab === '请选择') {
                this.show = true;
                this.text = '请选择地址类别';
                return;
            } else if (!(/1[34578]\d{9}$/).test(this.list.mobile)) {
                this.show = true;
                this.text = '请输入正确的手机号';
                return;
            }
            ...
            // console.log(addlist)
            if (this.$route.params.id) {
                api.addressUpdateActivity(this, this.list, back => {
                    if (back.success == 1) {
                        this.$router.replace({
                            path: '/groupAddress'
                        })
                ...
                api.addAddressActivity(this, this.list, back => {
                    if (back.success == 1) {
                        this.$router.replace({
                            path: '/groupAddress'
                        })
        ...
        getprovince() {
            for (var i = 0; i < this.jsonData.length; i++) {
                this.provinceArr.push(this.jsonData[i].name)
            };
            this.getcity();
        },
        getcity() {
            this.cityArr.splice(0, this.cityArr.length);
            this.areaArr.splice(0, this.areaArr.length);
            for (var k = 0; k < this.jsonData.length; k++) {
                if (this.list.province === this.jsonData[k].name) {
                    for (var j = 0; j < this.jsonData[k].city.length; j++) {
                        this.cityArr.push(this.jsonData[k].city[j].name)
                    }
                    this.selectArea = this.jsonData[k].city;
                    break;
                }
            };
            this.getarea();
        ...
        getarea() {
            for (var i = 0; i < this.cityArr.length; i++) {
                if (this.selectArea[i].name === this.list.city) {
                    this.areaArr = this.selectArea[i].area
                }
            };
...
.address {
    ...
    margin: 0 auto;
    border-radius: 10px;
    box-shadow: 2px 2px 20px #9b927f;
    p {
        float: left;
        margin-right: 20px;
        line-height: 52px;
