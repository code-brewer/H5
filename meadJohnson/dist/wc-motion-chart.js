    ...
    WcChart.prototype = {
        ...
        initData: function() {
            ...
            var maxData = Math.max.apply(Math, this.data),
                ...
                itrDay = new Date(this.day.getTime()),
                // yMaxUint = maxData <= this.yAxis ? this.yAxis : 
                //     Math.ceil((maxData - this.yAxis) / this.yAxisIc) * this.yAxisIc / 2 + this.yAxis;
                yMaxUint = 20800;
            ...
            中间的基准线--找出1W基准线的y坐标
             */
            this.chartLowerLine = this.rectChartCenter.y - 20 + 20800 * this.rectChartCenter.height / yMaxUint;
            this.chartMeanLine = this.rectChartCenter.y - 20 + 10000 * this.rectChartCenter.height / yMaxUint;
            this.chartHighLine = this.rectChartCenter.y - 20 + 2800 * this.rectChartCenter.height / yMaxUint;
            /* transfer data into points */
            var bool = true;
            this.points = [];
            for (var i = 0; i < this.data.length; i++) {
                if (this.data.length > 8) {
                    bool = i % 5 == 0 ? true : false;
                }
                this.points.push({
                    ...
                    y: this.rectChartCenter.y - 20 + this.data[i] * this.rectChartCenter.height / yMaxUint,
                    value: this.data[i],
                    time: this.sign[i],
                    text: i == 0 ? (itrDay.getMonth() + 1) + "月" + itrDay.getDate() : bool ? itrDay.getDate() : '',
                    radius: i == this.data.length - 1 ? 9 : 6,
                    textAlign: i == 0 ? 'left' : 'center',
                    rect: {
                        x: this.rectChartCenter.x + interval * (i - 0.5),
                        y: this.rectChart.y,
                        width: interval,
        ...
        drawHead: function() {
            ...
            // ctx.fillStyle = "white";
            // ctx.fillText(this.headText, this.rectHead.x, this.rectHead.y + fontSize + 20,
            //     this.rectHead.width);
            // ctx.textAlign = "right";
            ...
            ctx.moveTo(this.rectHead.x, this.chartHighLine);
            ctx.lineTo(this.rectHead.x + this.rectHead.width - 15, this.chartHighLine);
            ctx.stroke();
        ...
        drawChart: function() {
            ...
            // ctx.fillText("5:00", this.rectChartCenter.x + this.rectChartCenter.width + 20,
            //     this.rectChartCenter.y + this.chartFontSize / 3);
            ctx.fillText("05:00", this.rectChartCenter.x + this.rectChartCenter.width + 20,
                this.chartHighLine + this.chartFontSize / 3);
            ctx.stroke();
            ...
            ctx.globalAlpha = 0.6;
            ctx.font = (this.chartFontSize) + "px " + this.fontFamily;
            ctx.fillText("07:00", this.rectChartCenter.x + this.rectChartCenter.width + 20,
                this.chartMeanLine + this.chartFontSize / 3);
            ctx.beginPath();
            ctx.lineWidth = 1;
            ...
            ctx.globalAlpha = 0.6;
            ctx.font = (this.chartFontSize) + "px " + this.fontFamily;
            ctx.fillText("10:00", this.rectChartCenter.x + this.rectChartCenter.width + 20,
                this.chartLowerLine + this.chartFontSize / 3);
            ctx.beginPath();
            ctx.lineWidth = 1;
            ctx.dashedLineTo(this.rectChartCenter.x, this.chartLowerLine,
                this.rectChartCenter.x + this.rectChartCenter.width, this.chartLowerLine, [7, 4]);
            ...
            for (var i = 0; i < this.points.length; i++) {
                ...
                var x = i == 0 ? this.points[i].x - 20 : this.points[i].x;
                ctx.fillText(this.points[i].text, x,
            ...
            for (i = 0; i < this.points.length; i++) {
                ...
                /* draw line */
                if (i != 0 && this.points[i].time !== '' && this.points[i - 1].time !== '') {
                    ctx.save();
                    ctx.beginPath();
                    ctx.lineWidth = 2;
        ...
        /* draw animation step */
        drawStep: function(v) {
            if (this.animationPoint == null || this.animationPoint < 0 || this.animationPoint >= this.points.length || this.isDataLoad == false) {
                return;
            }

            var ctx = this.ctx,
                point = this.points[this.animationPoint],
                lastY = this.rectChartCenter.y - this.chartFontSize * .5,
                beginY = point.y,
            ...
            //绘制打卡时间文本
            ctx.fillText(point.time, point.x, textY);
            ctx.restore();
        },
