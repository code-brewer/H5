...
var FileUploadComponent = Vue.extend({
    template: '<div class="{{ class }}"><input type="file" name="{{ name }}" id="{{ id || name }}" accept="{{ accept }}" v-on:click="fileInputClick" v-on:change="fileInputChange" multiple="{{ multiple }}"><slot></slot><button type="button" class="btn btn-default" v-on:click="clickInput" v-show="displaybtn">上传</button></div>',
    props: {
        class: String,
        name: {
            type: String,
