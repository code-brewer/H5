<template>
	<h1 class="manage">开销户结果管理</h1>
	<div class="oneAccount" style="margin: 35px 50px;background-color:#fff;border:1px solid #ccc;"><span style="color:#ff0000;">注：2/3G批量开销户后，可在列表中导出明细查看是否开销户成功。</span></div>
		
	
	<div class="memberBody">
		<div class="memberTop">
		<span style="margin-right:10px;">类型</span>
			<select class="selectChoose" v-model='resultData.type' style="font-size:12px;">
				<option value="">全部</option>
				<option value="REG">批量开户</option>
				<option value="CANCEL">批量销户</option>
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;
        <a class="check" @click="resultCheck()">查找</a>
    <!-- </div> -->
		</div>
		...
		<table class="AcountTable">
			<tbody>
				<tr style="background-color:#faa354;">
					...
					<td class="rightTd" style="width:60%;">文件创建时间</td>
					<td class="rightTd" style="width:20%;">下载查看</td>
					
				</tr>
				<tr  v-for="list in lists">
					<td class="rightTd"> {{list.type == 'CANCEL'?'批量销户':'批量开户'}}</td>
					<td class="rightTd">{{list.createtime}}</td>
					<td class="rightTd" @click="download(list.id)"><a class="downbtn">
					导出明细</a></td>
	...
	export default {
		...
		methods:{
			...
			resultCheck(){
				var that = this;
				this.isLoad = true;
				this.resultData.page = this.pageConfig.curpage;
				var starttime = new Date(that.starttime).getTime() || ''
            	that.resultData.starttime = starttime
            	var endtime = new Date(that.endtime).getTime() || ''
            	that.resultData.endtime = endtime
				api.logfileSelect(this,this.resultData,(back)=>{
					if(back.resCode=="0"){
						this.isLoad = false;
						back.repBody.list.forEach(value=>{
								value.createtime = Dateutil.format(new Date(value.createtime), 'yyyy-MM-dd HH:mm:ss')
							})
			...
			download(id){
				window.location.href = basePath + 'logfile/download?id='+id
			},
			backlogin(){
