...
// var DateUtils = {
    ...
    formatEmoij: function(comment){
        ...
        if (text && text.length) {
            text.forEach(item => {
                comment = comment.replace(item, '<span style="display: inline-block; width: 60px; height: 56px;  vertical-align: middle; background-image: url(' + emoji.data[item].img + '); background-position: center center; background-repeat: no-repeat;"></span>');
            })
        }
        return comment;
    }
