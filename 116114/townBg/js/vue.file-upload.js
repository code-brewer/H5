...
var FileUploadComponent = Vue.extend({
  template: '<div class="{{ class }}"><button style="width:78px;height:30px;position:relative;">上传<input style="position:absolute;top:0;left:0;width:100%;height:100%;opacity: 0;" type="file" name="{{ name }}" id="{{ id || name }}" accept="{{ accept }}" v-on:click="fileInputClick" v-on:change="fileInputChange" multiple="{{ multiple }}"><slot></slot></button></div>',
  props: {
    class: String,
    name: {
      type: String,
      ...
      return new Promise(function(resolve, reject) {
        ...
        xhr.onreadystatechange = function() {
          ...
          if (xhr.status < 400) {
            ...
            this.$dispatch('onFileUpload', file, res,this.params);
            resolve(file);
          } else {
