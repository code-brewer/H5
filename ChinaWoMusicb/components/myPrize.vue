<template>
	<modal :show.sync="show" :bg="false">
		 <section class="prize-panel" style="background:url(b/images/bg-border-2.png);">
		 		<div class="title">
		 				我的奖品
		 		</div>
		 		<ul class="data-ul">
		 			<li v-for="list in prizeList" style="background: url(b/images/libg-wdjp.png);">
						 <p class="phone">{{!!list.mobile?list.mobile:''}}</p>
						 <p class="prize-name">{{list.pname}}</p>
						 <a class="btn" @click="goPrize(list.gid,list.mobile)" :style="!list.mobile?'background:url(b/images/btn-cfm-2.png)':'background:url(b/images/btn-cfm-3.png)'" >
						 	{{!list.mobile?'去领奖':'已领取'}}
						 </a>
						 <img class="prize-img" :src="$root.activityBasePath+list.imgurl">
					</li> 
		 		</ul>
		 		<img class="close" :src="'b/images/close-2.png'" @click="show=false"/>
		 </section>
	</modal>
</template>
<style lang="sass">
