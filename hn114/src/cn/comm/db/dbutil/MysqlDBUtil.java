...
public class MysqlDBUtil {
	...
	private static final Log log = LogFactory.getLog(CommonParams.LOG_SYS);
		...
		try {
			...
			while (rs.next()) {
				...
				for (int i = 1; i <= columnCount; i++) {
					...
					// 字符
					if (columnType.toUpperCase().equals("CHAR") || columnType.toUpperCase().equals("VARCHAR") || columnType.toUpperCase().equals("VARCHAR2")
							|| columnType.toUpperCase().equals("NVARCHAR") || columnType.toUpperCase().equals("NVARCHAR2")) {
						stringValue = rs.getString(i);
						setColName(rsmd.getColumnLabel(i), toFormatString(stringValue), map);
					}

					// 数据类型
					if (columnType.toUpperCase().equals("NUMBER") || columnType.toUpperCase().equals("INT") || columnType.toUpperCase().equals("FLOAT")) {
						decimalValue = rs.getBigDecimal(i);
						//getColumnLabel --getColumnName
						setColName(rsmd.getColumnLabel(i), decimalValue + "", map);
					}

					// 日期类型
					if (columnType.toUpperCase().equals("DATE") || columnType.toUpperCase().equals("DATETIME") || columnType.toUpperCase().equals("TIMESTAMP")) {
						dateValue = rs.getDate(i);

						SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm");
						if (dateValue != null && !dateValue.equals("")) {
							setColName(rsmd.getColumnLabel(i), df.format(rs.getTimestamp(i)), map);
						} else {
	 ...
	 * @throws Exception
	 */
	public List queryToListByProc(Connection conn, String procName, String params, String[] args) throws Exception {
		...
		try {
			...
			for (int i = 0; i < count; i++) {
				...
				if (param.indexOf("'") != -1) {// 字符
					...
					stmt.setBigDecimal(i + 1, new BigDecimal(paramArr[i]));
				}
			...
			while (rs.next()) {
				...
				for (int i = 1; i <= columnCount; i++) {
					...
					// 数据类型
					if (columnType.toUpperCase().equals("NUMBER") || columnType.toUpperCase().equals("INT") || columnType.toUpperCase().equals("FLOAT")
							|| columnType.toUpperCase().equals("DOUBLE") || columnType.toUpperCase().equals("DECIMAL")) {
						decimalValue = rs.getBigDecimal(i);
						setColName(rsmd.getColumnLabel(i), decimalValue + "", map);
					}

					// 日期类型
					if (columnType.toUpperCase().equals("DATE") || columnType.toUpperCase().equals("TIMESTAMP") || columnType.toUpperCase().equals("DATETIME")) {
						dateValue = rs.getDate(i);
						...
						if (strings.indexOf(rsmd.getColumnLabel(i)) != -1) {// 如果需要带时、分、秒的日期
							...
							if (dateStr != null && dateStr.length() > 2) {
								setColName(rsmd.getColumnLabel(i), dateStr.substring(0, dateStr.length() - 2), map);
							}
	 ...
	 * @throws SQLException
	 */
	public Map callDMLProc(Connection conn, String procName, String params) throws SQLException {
	...
	private String generateStrings(String[] arr) {
		...
		if (arr != null && arr.length > 0) {
			...
			strings = (s == null || s.length() < 1) ? "" : s.substring(0, s.length() - 1);
		}
		return strings;
	}
	 ...
	 * @throws SQLException
	 */
	public boolean insertByBatchStat(Connection conn, Object[][] parm, String sql) throws SQLException {
		boolean flag = false;
		Statement stmt = null;
		try {
