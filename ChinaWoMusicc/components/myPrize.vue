<template>
	<modal :show.sync="show" :bg="false">
		 <section class="prize-panel" style="background:url(c/images/bg-border-2.png);">
		 		...
		 		<ul class="data-ul">
		 			<li v-for="list in prizeList" style="background: url(c/images/libg-wdjp.png);">
						 <p class="phone">{{!!list.mobile?list.mobile:''}}</p>
						 <p class="prize-name">{{list.pname}}</p>
						 <a class="btn" @click="goPrize(list.gid,list.mobile)" :style="!list.mobile?'background:url(c/images/btn-cfm-2.png)':'background:url(c/images/btn-cfm-3.png)'" >
						 	{{!list.mobile?'去领奖':'已领取'}}
						 </a>
