    ...
    */
    var QD = jeDt.query = function() {
        function r(c, g) { g = g || document;
            ...
            var d = c.match(z),
                ...
                b = !e && (a.match(v) || k)[1],
                a = c.match(/\[(?:[\w\-_][^=]+)=(?:[\'\[\]\w\-_]+)\]/g);
            if (f && !a && !b && g.getElementsByClassName) b = m(g.getElementsByClassName(f));
        ...
        function m(c) {
            try {
                return Array.prototype.slice.call(c) } catch (g) {
        ...
        function p(c, g, d) {
            ...
            for (var b = [], e = -1, f = (a.match(t) || k)[1], h = !f && (a.match(u) || k)[1], a = !f && (a.match(v) || k)[1], m = -1, q, l, n, a = a && a.toLowerCase(); q = g[++m];) { l = q.parentNode;
                do
                    if (n = (n = (n = !a || "*" === a || a === l.nodeName.toLowerCase()) && (!f || l.id === f)) && (!h || RegExp("(^|\\s)" + h + "(\\s|$)").test(l.className)), d || n) break;
    ...
    var config = {
        ...
        InitDate = function(options) {
            var that = this,
                newConf = window.JSON.parse(window.JSON.stringify(config));
    ...
    InitDate.prototype = {
        ...
        events: function(that, opts, self, Tms) {
            var yPre = QD(Cell + " .yearprev")[0],
                yNext = QD(Cell + " .yearnext")[0],
                mPre = QD(Cell + " .monthprev")[0],
                mNext = QD(Cell + " .monthnext")[0],
                newDate = new Date(),
            ...
            jeDt.on(QD(Cell + " .jedatebot .jedateok")[0], "click", function(ev) {
                ...
                if (!isformat) {
                    var okTimeArr = [],
                        okTime = [parseInt(jeDt.attr(jedateyear, "data-year")), parseInt(jeDt.attr(jedatemonth, "data-month")), Tms[2]];
    ...
    jeDt.creatlink = function(lib) {
        ...
        link.type = 'text/css';
        link.rel = 'stylesheet';
        console.log(jeDt.getPath)
        link.href = './js/timejs/skin/' + lib + '.css';
        link.id = 'jeDateSkin';
