<template>
				...
				<div class="right-buttons" v-if="$root.isLogin">
					...
					<menu-clip for="ams-menu-button">
						<ul>
							...
							<li class="menu-item" v-if="!isHome && userinfo.level == 1">
	...
	<login-modal :show.sync="loginModalShow" :flag.sync="flag"></login-modal>
</template>
...
<script>
	...
	export default {
		...
		watch:{
			flag(v){
				this.userinfo = JSON.parse(sessionStorage.getItem('ams_v2_userinfo')) ? JSON.parse(sessionStorage.getItem('ams_v2_userinfo')) : {level:2} ;
			},	
		},
		ready() {
			var self = this;
		...
		methods: {
			...
			logout() {
				...
				sessionStorage.removeItem('ams_v2_userinfo');
