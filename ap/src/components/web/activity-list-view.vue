<template>
	...
	<div class="activity-list-view-contaier">
		<!-- 平铺 -->
		<div v-if="viewModel == 'horizontal'" class="activity-list-view-list horizontal">
			<ul>
				<li class='activity-list-view-item' v-for="(index, list) in lists">
					<div class="activity-list-view-item-box">
						<div class="box-top">
							<div class="mask"></div>
							<img class="img" :src="list.url.split(';').length==2?list.url.split(';')[1]:list.url.split(';')[0]">
							<img v-if="list.showCodeimg" class="code-img" :src="list.codeImgurl" transition="code-img">
						</div>
						...
						<div class="post-data">
							<div class="post-data-left">
								<i class="code-img" @click="createCode(index, list)" :class="{selected: list.showCodeimg}"></i>
							</div>
							<div class="post-data-right">
								<ul>
									...
									<li class="post-data-item" @click="doAction('status', list)">
										<i></i>
										<span>{{list.status == "3" ? "下线" : "上线"}}</span>
									</li>
									<li class="post-data-item" @click="doAction('del', list),confirmShow=true">
										<i></i>
										<span>删除</span>
									</li>
								</ul>
						...
						<div class='post-act'>
							<ul>
								<li title="编辑" class="post-act-item" @click="doAction('edit', list)"><i></i></li>
								<li title="复制URL" class="post-act-item" @click="doAction('url', list)"><i></i></li>
								<li title="活动数据" class="post-act-item" v-link="{name:'dataStatistics',params:{aid:list.aid, modaln:list.title}}"><i></i></li>
							</ul>
						</div>
					</div>
				</li>
        ...
        <pager :pagenum.sync="selectActivityData.pageNum" :pagesize="selectActivityData.pageSize" :total.sync="selectActivityData.total"></pager>
		<modal :show.sync="urlData.show" class="dia">
			<div class="modal-header">
				<p class="title">复制URL</p>
			</div>
			<div class="modal-body" style="text-align: center;">
				<p class="url-text">{{urlData.url}}</p>
				<em style="color: red;">{{urlData.errMsg}}</em>
			</div>
			<div class="modal-footer">
				<button id="copy-url-btn"  @click="copys" data-clipboard-text="{{urlData.url}}" class="act-btn copy">复制</button>
				<button class="act-btn" @click="urlData.show = false">取消</button>
			</div>
		</modal>
...
<script>
	...
	export default {
		...
		ready() {
			...
			// 	var clip = new ZeroClipboard(document.getElementById("copy-url-btn"));
		...
		methods: {
			...
			//获取活动url
			getActivityUrl(aid) {
				return basePath_2.replace('apm', 'apu') + 'login.jsp?aid=' + aid;
			},
			//生成二维码图片
			createQrcode(aid) {
				return 'http://qr.liantu.com/api.php?w=320&text=' + this.getActivityUrl(aid);
			},
			//干活
			doAction(_name, item) {
				if("edit" == _name) {
					var pname = window.location.pathname.replace("template", "activity");
					var href = window.location.origin + pname + "?jspath=" + item.jspath + "&aid=" + item.aid+'&type=' + item.type + "#!/";
					window.open(href);
				} else if("url" == _name) {
					//复制url				
					this.urlData.url =  this.getActivityUrl(item.aid);;
