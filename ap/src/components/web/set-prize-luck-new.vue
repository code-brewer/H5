<template>
	<div class="form-group" style="height: 505px;overflow-x: hidden;overflow-y: auto;">
		...
		<div class="label-inline">
			...
			<radio :model.sync="chanceIsTop" style="margin-right: 15px;" name="chancetop" value="0">就一次</radio>
			<radio :model.sync="chanceIsTop" name="chancetop" value="yes">限制</radio>
			<label style="margin-left: 15px;" v-if="chanceIsTop=='yes'">每人每日有<input type="number" style="width: 40px;margin: 0 5px;" v-model="chanceNum" @blur="checkInput(chanceNum)"/>次抽奖机会</label>
...
<script>
	import prize from './prize-new.vue'
	...
	export default {
		...
		events: {
			changePlayer() {
				var player = [];
				for(var i = 0; i < this.configData.player.length; i++) {
					player[i] = this.configData.player[i].toString().split("_")[0];
				}
				this.$set("configData.player", player);
			},
			// 缓存保存已选中的奖品
			saveSelectedList() {
				...
				for(var j = 0; j < prizeList.length; j++) {
					...
					newList[0].prizeList[j].winrate = prizeList[j].winrate*100;
					allWinrate += newList[0].prizeList[j].winrate;
					for(var k = 0; k < gids.length; k++) {
						var gid = newList[0].prizeList[j].gids[k].gid;
						newList[0].prizeList[j].gids[k] = gid
					}
				}
				this.$set("activityConfig.allWinrate", allWinrate);
				if(newList.length == 1) {
					var newObj = {
						"date": "1111-11-11",
						"prizeList": [{
				...
				for(var i = 0; i < this.configData.player.length; i++) {
					if(this.chanceIsTop == "yes"){
						this.configData.player[i] = this.configData.player[i] + "_" + this.chanceNum;
					}else if(this.chanceIsTop == 0){
						this.configData.player[i] = this.configData.player[i] + "_" + 0;
					}else{
						this.configData.player[i] = this.configData.player[i];
					}
					// this.configData.player[i] = [this.chanceIsTop == "yes" ? this.configData.player[i] + "_" + this.chanceNum : this.configData.player[i]];
				}
				this.$set("activityConfig.playerNum", this.configData.player);
			},
			// 初始化编辑已选中奖品列表
			changeSelectList(list) {
				console.log(window.store.configData.player+"---");
				...
				for(var j = 0; j < prizeList.length; j++) {
					...
					this.selectedList[0].prizeList[j].winrate = this.selectedList[0].prizeList[j].winrate/100;
					if(gids.length != 0) {
				...
				console.log(this.configData.player+'0000');
				this.$set("activityConfig.playerNum", this.configData.player);
				...
				var player = [];
				for(var i = 0; i < this.configData.player.length; i++) {
					player[i] = this.configData.player[i].split("_")[0];
				}
				this.$set("configData.player", player)
			},
		}
