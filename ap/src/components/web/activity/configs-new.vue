<template>
	<div class="ams-configs">
		...
		<div class="config-contents"
				...
				<tabs :header="pageConfig.header"
					...
					<div slot="basic">
						...
						<div class="label-inline"
							...
							<datepicker v-ref:dpstart
							            :value.sync="configData.starttime"
							            :show-input="false"
							            format="yyyy-MM-dd"
							            placeholder="年-月-日"
							            width="120px"></datepicker>
							<span style="width: 50px; text-align: center;">至</span>
							<datepicker v-ref:dpend
							            :value.sync="configData.endtime"
							            :show-input="false"
							            format="yyyy-MM-dd"
							            placeholder="年-月-日"
							            width="120px"></datepicker>
						...
						<div class="label-inline">
							...
							<checkbox :model.sync="activityConfig.participant.show"
							          :disabled="activityConfig.participant.disabled">显示</checkbox>
						</div>
						<div class="label-inline"
						     style="margin-top: 0;"
						     v-show="activityConfig.participant.show">
							<span class="onse"></span>
							<span>在实际参与人数上增加</span>
							<input type="text"
							       class="form-control"
							       v-model="activityConfig.participant.addValue"
							       style="width: 80px;margin: 0 10px;"
							       min="0">
							<span>人</span><small class="gray">(只展示不计入统计)</small></div>
						<div class="label-inline">
							<span class="onse">奖品数量：</span>
							<checkbox :model.sync="activityConfig.atNum.show"
							          :disabled="activityConfig.atNum.disabled">显示</checkbox>
							<small class="gray">(显示剩余的奖品数量)</small>
						</div>
						<div class="label-inline"
						     v-show="activityConfig.atNum.show">
							<span class="onse"></span>
							<input type="text"
							       class="form-control"
							       v-model="activityConfig.atNum.text.valueL"
							       style="width: 80px;text-align: center; margin: 0px 10px;">
							<p style="width: 50px;margin: 0; text-align:center;">{{configData.gidcnt}}</p>
							<question>奖品数量不可手动更改，为设置的奖品数</question>
							<input type="text"
							       class="form-control"
							       v-model="activityConfig.atNum.text.valueR">
						</div>
						...
						<div class="label-inline">
							...
							<checkbox :checked.sync="true"
							          :model.sync="activityConfig.logo.show">显示</checkbox>
						</div>
						<div class="label-inline">
							<span class="onse">标题：</span>
							<checkbox :checked.sync="true"
							          :model.sync="activityConfig.banner.show">显示</checkbox>
						</div>
						<div class="label-inline">
							<span class="onse">奖品状态：</span>
							<checkbox :checked.sync="true"
							          :model.sync="activityConfig.prizeList.typeShow">显示</checkbox>
						</div>
						<div class="label-inline"
						     :style="{'align-items': 'flex-start'}">
							<span class="onse onse-has-red">规则编辑：</span>
							<div class="rule-content">
								<div class="rule-tabs">
									<a href="javascript:;"
									   :class="{'active': !isPrizeRule}"
									   @click="isPrizeRule=false">活动规则</a>
									<a href="javascript:;"
									   :class="{'active': isPrizeRule}"
									   @click="isPrizeRule=true">领奖规则</a>
								</div>
								<div>
									<web-editor v-if="!isPrizeRule"
									            :textarea-editor="'activityRule'"
									            :rule-content.sync="activityConfig.ruleCon.value">
									</web-editor>
									<web-editor v-else
									            :textarea-editor="'priczeRule'"
									            :rule-content.sync="activityConfig.prizeRule.value">
									</web-editor>
								</div>
							</div>
						</div>
					...
					<div slot="more">
						<div class="label-inline"
							...
							<checkbox :checked="false"
							          :model.sync="activityConfig.bannerPage.enable">使用</checkbox>
							<checkbox :checked="false"
							          :model.sync="activityConfig.bannerPage.show"
							          style="margin-left: 20px;">预览</checkbox>
						</div>
						<div class="label-inline"
						     style="align-items: flex-start;">
							<span class="onse"></span>
						...
						<div class="label-inline"
							...
							<checkbox :model.sync="activityConfig.qeCode.show"
							          :disabled="activityConfig.qeCode.disabled">显示</checkbox>
						</div>
						<div class="label-inline"
						     style="align-items: flex-start;">
							<span class="onse"></span>
			...
			<!-- 条件设置 -->
			<div class="content"
			     id="condition-config-content"
			     v-show="currTabHas==configsTabs[1].url">
				<div class="label-inline"
				     v-bind:class="{'edit-disabled': isOnline}">
					<span class="onse">参与对象：</span>
					<checkbox :model.sync="configData.player"
					          value="W">微信号</checkbox>
					<checkbox :model.sync="configData.player"
					          value="M"
					          v-if="isMobile != 'false'"
					          style="margin-left: 10px">手机号</checkbox>
				...
				<div class="label-inline"
					...
					<a class="btn btn-primary"
					   @click="showQuestionBank=true">我的题目</a>
			...
			<div class="content"
			     v-bind:class="{'edit-disabled': isOnline}"
			     id="prize-config-content"
			     v-show="currTabHas==configsTabs[2].url">
				<component :is="currentView"></component>
			</div>
			<!-- 高级配置 -->
			<!-- <div class="content" v-show="currTabHas==configsTabs[3].url && type == 'old'">
...
import qrCode from '../../web/qr-code-new.vue'
...
import setPrizeLuck from '../../web/set-prize-luck-new.vue'
import questionBank from '../../web/question-bank.vue'
import webEditor from '../../web-base/WebEditor'
...
export default {
	...
	ready() {
		this.$on("save", () => {
			...
			// 活动规则为空
			if (this.activityConfig.ruleCon.value.trim() == "") {
				this.snackbar_active = true;
				this.snackbar_msg = "活动规则不能为空";
				this.disabled = false;
				return;
			}

			// 领奖规则为空
			if (this.activityConfig.prizeRule.value.trim() == "") {
				this.snackbar_active = true;
				this.snackbar_msg = "领奖规则不能为空";
				this.disabled = false;
				return;
			}
			...
			//奖品概率总和
			if (this.activityConfig.allWinrate > 10000) {
				this.snackbar_active = true;
				this.snackbar_msg = "奖品概率总和不能大于100%";
				this.disabled = false;
				return;
			}
	...
	watch: {
		'configData.endtime'(v, o) {
			...
			// 结束日期小于global.js下的结束日期时，进入条件下改变结束日期为当前日期+1
			if (utils.formatDate(newDate, "yyyy-MM-dd") < this.configData.starttime) {
				this.snackbar_active = true;
				this.snackbar_msg = '活动结束日期不能小于开始日期.';
				this.configData.endtime = o; // 结束日期为当前日期+1
			}
		},
		'configData.starttime'(v, o) {
			this.configData.starttime = v.split(" ")[0];
			var newDate = new Date(this.configData.starttime);
			newDate.setDate(newDate.getDate());
			// if (utils.formatDate(newDate, "yyyy-MM-dd") < v || this.configData.endtime < utils.formatDate(new Date(), "yyyy-MM-dd")) {
			if (this.configData.endtime < utils.formatDate(newDate, "yyyy-MM-dd")) {
				this.snackbar_active = true;
				this.snackbar_msg = '活动开始日期不能大于结束日期.';
