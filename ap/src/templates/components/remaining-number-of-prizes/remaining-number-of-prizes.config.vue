<template>
	<div class="wap-remaining-number-of-prizes" v-show="activityConfig.atNum.show">
		...
		<p class="number-countent" v-el:number :style="{'color':activityConfig.atNum.text.color,'font-size':activityConfig.atNum.text.fontSize}">
			...
			<span v-el:count class="count" :style="{'color':activityConfig.atNum.num.color,'font-size':activityConfig.atNum.num.fontSize}">{{configData.prize_cnt}}</span> {{activityConfig.atNum.text.valueR}}
		</p>
...
<script>
	...
	export default {
		...
		ready() {
			...
			window.store.configData.firstPrizeCnt = window.store.configData.prize_cnt;
			this.init();
		}
	}
</script>