<template>
	<div class="lucky-modal-btn"
	     v-el:luckybtn
	     :style="{'background-image':(activityConfig.lucky.btnImg!=''?'url('+activityConfig.lucky.btnImg+')':1), 'background-size':'100%', 'background-repeat':'no-repeat', 'height':activityConfig.lucky.btnImgH}">
		<edit-btn></edit-btn>
		<modal :show.sync="showForm"
			...
			<div class="modal-panel">
				...
				<upload-image :img-url.sync="activityConfig.lucky.btnImg"
				              :config="uploadConfig"
...
export default {
	...
	methods: {
		init() {
			var btn = document.defaultView.getComputedStyle(this.$els.luckybtn);
			this.activityConfig.lucky.btnImg = btn.backgroundImage.split('"')[1];
			this.setImage = new Image();
			this.setImage.src = this.activityConfig.lucky.btnImg;
			this.setImage.onload = () => {
				var l = this.setImage.width / this.setImage.height;
				this.activityConfig.lucky.btnImgH = (this.$els.luckybtn.offsetWidth / (l * 12)) + 'rem';
			}
		},
