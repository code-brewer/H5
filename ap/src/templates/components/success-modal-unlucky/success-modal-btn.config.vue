<template>
	<div class="unlucky-modal-btn"
	     v-el:unluckybtn
	     :style="{'background-image':(activityConfig.unlucky.btnImg!=''?'url('+activityConfig.unlucky.btnImg+')':1), 'background-size':'100%', 'background-repeat':'no-repeat', 'height':activityConfig.unlucky.btnImgH}">
		<edit-btn></edit-btn>
		<modal :show.sync="showForm"
			...
			<div class="modal-panel">
				...
				<upload-image :img-url.sync="activityConfig.unlucky.btnImg"
				              :config="uploadConfig"
...
export default {
	...
	watch: {
		'activityConfig.unlucky.btnImg'(v) {
			this.setImage.src = v;
		}
	...
	methods: {
		init() {
			var btn = document.defaultView.getComputedStyle(this.$els.unluckybtn);
			this.activityConfig.unlucky.btnImg = btn.backgroundImage.split('"')[1];
			this.setImage = new Image();
			this.setImage.src = this.activityConfig.unlucky.btnImg;
			this.setImage.onload = () => {
				var l = this.setImage.width / this.setImage.height;
				this.activityConfig.unlucky.btnImgH = (this.$els.unluckybtn.offsetWidth / (l * 12)) + 'rem';
			}
		},
