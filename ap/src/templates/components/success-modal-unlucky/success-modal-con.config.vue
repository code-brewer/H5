<template>
	<div class="unlucky-modal-title">
		<edit-btn></edit-btn>
		<p  v-el:unluckycon :style="{'font-size':activityConfig.unlucky.conSize,'color':activityConfig.unlucky.conColor}">{{activityConfig.unlucky.con}}</p>
			    	...
			    	<div class="spane">
			    		...
			    		<div class="nr">
			    			...
			    			<p class="col" style="line-height: 1.8;">
			    				...
			    				<input type="text" class="form-control" v-model="activityConfig.unlucky.con" style="margin-left: 24px; display: inline-block;width: 200px;"/>
			    			</p>
			    			 <p class="col">
			    			 	<span>
			    			 		颜色：
			    			 	</span>
				    			<i :style="{'background-color':activityConfig.unlucky.conColor}" @click.stop="openColor"></i>
...
<script>
	...
	export default {
		...
		watch:{
			titleSize(v){
			 	this.activityConfig.unlucky.conSize = v + "rem";
			},
			'activityConfig.unlucky.conSize'(v){
		...
		methods:{
			colseColor(){
				if(this.selectColor)
				{
					this.activityConfig.unlucky.conColor=this.colors.hex;
				...
				if(!this.selectColor)
					{
						...
						this.colors.hex = this.activityConfig.unlucky.conColor;
					}
			},
			init(){
 				var rule_text = document.defaultView.getComputedStyle(this.$els.unluckycon);
				this.activityConfig.unlucky.conColor = rule_text.color;
