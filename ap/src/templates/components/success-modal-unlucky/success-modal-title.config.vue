<template>
	<div class="unlucky-modal-title">
		<edit-btn></edit-btn>
		<p  v-el:unluckytitle :style="{'font-size':activityConfig.unlucky.titleSize,'color':activityConfig.unlucky.titleColor}">{{activityConfig.unlucky.title}}</p>
			    	...
			    	<div class="spane">
			    		...
			    		<div class="nr">
			    			...
			    			<p class="col" style="line-height: 1.8;">
			    				...
			    				<input type="text" class="form-control" v-model="activityConfig.unlucky.title" style="margin-left: 24px; display: inline-block;width: 200px;"/>
			    			</p>
			    			 <p class="col">
			    			 	<span>
			    			 		颜色：
			    			 	</span>
				    			<i :style="{'background-color':activityConfig.unlucky.titleColor}" @click.stop="openColor"></i>
...
<script>
	...
	export default {
		...
		watch:{
			titleSize(v){
			 	this.activityConfig.unlucky.titleSize = v + "rem";
			},
			'activityConfig.unlucky.titleSize'(v){
		...
		methods:{
			colseColor(){
				if(this.selectColor)
				{
					this.activityConfig.unlucky.titleColor=this.colors.hex;
				...
				if(!this.selectColor)
					{
						...
						this.colors.hex = this.activityConfig.unlucky.titleColor;
					}
			},
			init(){
 				var rule_text = document.defaultView.getComputedStyle(this.$els.unluckytitle);
				this.activityConfig.unlucky.titleColor = rule_text.color;
