<template>
	<div class="app-wrap"
	     :class="direct"
	     :style="{'background-image':activityConfig.common.bgUrl?'url('+activityConfig.common.bgUrl+')':1,'background-color':activityConfig.common.bg}">
...
export default {
	...
	ready() {
		...
		// 补上url?user=xx&channleID=xx&contentid=xx; user: phone, openid
		let channelid = utils.getQueryStringByName('channle')
		let contentid = this.activityConfig.game.contentId
		let user = window.identity
		this.musicUrl = this.activityConfig.game.musicUrl[this.activityConfig.game.fileType] + '?user=' + user + '&channleID=' + channelid + '&contentid=' + contentid

		let $audio = document.getElementById('music')

		$audio.addEventListener('ended', () => {
			// 结束时暂停cd转动
			this.$broadcast('playEnd')
		})
		// 绑定播放歌曲
		this.$on('playMusic', function () {
			$audio.load()
			$audio.play()
	...
	components: {
		...
		'banner-page': require('../../components/banner-page/banner-page.vue'),
	},
	events: {
