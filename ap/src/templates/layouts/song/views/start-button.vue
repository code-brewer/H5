...
export default {
	...
	methods: {
		goTo(evt) {
			...
			let starttime = new Date((this.$root.configData.starttime).replace(/-/g, '/')).getTime();
			let endtime = new Date((this.$root.configData.endtime).replace(/-/g, '/')).getTime();
			...
			Statistics.startBtn() // 统计开始按钮, 统计答题页pv/uv
