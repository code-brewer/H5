				...
				<table class="table table-bordered table-striped">
					...
					<tbody>
						<tr v-for="list in awradList">
                            ...
                            <td>{{list.weixin}}</td>
                            <td>{{list.usernick}}</td>
                            <td>{{list.sex == 1 ? '男' : list.sex == 2 ? '女' : '未知'}}</td>
                            <td>{{list.area}}</td>
							<td>{{list.mobile}}</td>
							<td>{{list.pname}}</td>
							<td>{{list.sid}}</td>
	...
	export default {
		...
		data() {
			return {
				...
				stime: '',
				etime: '',
				toExcel: 'acceptLog/newWinerPrizeExportToExcel?proNo=10004&reqTime=2016-12-09&aid=13&status=1&starttime=2016-11-24&endtime=2016-11-25',
			}
		},
		ready() {
			this.$root.fun = [];
		...
		methods: {
			...
			selectWinerPrize() {
				...
				this.toExcel = 'acceptLog/newWinerPrizeExportToExcel?proNo=10007&reqTime=2016-12-09&aid=' + this.$route.params.aid + '&starttime=' + this.stime + '&endtime=' +  this.etime;
